...
css=$(grep -o '/_next/static/css/[^"]*\.css' docs/index.html | head -1)
...
echo "→ verify (waiting ~90s for the Pages build)"
sleep 90
git ls-remote origin main | cut -c1-7 | sed 's/^/  origin: /'
git log --oneline -1 | cut -c1-7 | sed 's/^/  local:  /'
css_live=$(curl -s https://sealofaudit.com/ | grep -o '/_next/static/css/[^"]*\.css' | head -1)
code=$(curl -s -o /dev/null -w '%{http_code}' "https://sealofaudit.com$css_live")
head=$(curl -s "https://sealofaudit.com$css_live" | head -c 12)
echo "  stylesheet $css_live -> HTTP $code (first bytes: $head)"
if [ "$code" != "200" ] || [ "${head:0:1}" = "<" ]; then
  echo "  ⚠️  CSS not serving. If origin has it (raw.githubusercontent) this is the"
  echo "     Cloudflare 404-poisoning case: purge the CF cache, or wait out the"
  echo "     4h max-age on the poisoned URL."
  exit 1
fi
